...
public interface MealRepository extends JpaRepository<MealEntity, Integer>{
	...
	@Transactional
	@Modifying
	@Query(value="UPDATE MealEntity m SET m.name=:name, m.price=:price, m.link=:link, m.piece=:piece,m.category=:category,m.restaurant=:restaurant WHERE m.meal_id=:meal_id")
	public void updateMeal(@Param("meal_id") int id,@Param("name") String name,@Param("price") double price,@Param("link") String link,@Param("piece") boolean piece,@Param("category") CategoryEntity category,@Param("restaurant") RestaurantEntity restaurant);
	...
	@Transactional
	@Modifying
	@Query(value="select m.meal_id, m.name, m.price, m.link, m.piece,m.restaurant_id, c.category_id, c.link as categoryLink, c.name as categoryName from meals m join category c on m.category_id=c.category_id ORDER BY meal_id DESC OFFSET ?1 ROWS FETCH NEXT 10 ROWS ONLY",nativeQuery = true)
